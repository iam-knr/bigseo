...
function parseRobotsTxt(content: string): RoboticRules[] {
  ...
  for (const rawLine of lines) {
    ...
    } else if (key === "sitemap" && currentBlock) {
      currentBlock.allow.push(value);
    }
  }
...
function isAllowed(path: string, rules: readonly RoboticRules[], bot: string): boolean | undefined {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const node of ldNodes) {
      ...
      try {
        const data = JSON.parse(jsonText) as { [key: string]: unknown } | { [key: string]: unknown }[];
  ...
  try {
    ...
    if (robotsRes.ok) {
      ...
      robotsFound = true;
      const rules = parseRobotsTxt(robotsRaw);
      ...
      for (const line of robotsRaw.split(/\r?\n/)) {
        const lower = line.split("#")[0].trim().toLowerCase();
        if (lower.startsWith("sitemap:")) {
          const url = lower.split(":")[1].trim();
